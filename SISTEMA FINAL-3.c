...
Factura *obtenerFactura(int *n); /* Obtiene un vector dinámico de productos */
...
char eliminarFactura(int codigoFactura); /* Eliminar el producto de código codigoProducto del archivo */
char eliminacionFisicaF(); /* Realiza la eliminación física de registros inválidos del archivo de productos */
...
void ventaC(){
	...
	printf(" ------------------------------------------------------------------------\n");
        ...
        printf("\n\tDesea comprar algún otro producto? [S/N]: ");
		leecad(respuesta, MAX);
...
void menuInsertarF(){
	...
	do {
		...
		printf("\n\tCódigo que tendra la factura para la lista: ");
		leecad(linea, MAX);
		...
		if (!existeFactura(codigoFactura, &factura)) {
			...
			printf("\n\tEl código %d para la factura ya existe.\n", codigoFactura);
			printf("\tNo puede ingresar dos facturas distintas con el mismo código.\n");
		}
...
void menuEliminarF(){
	...
	do {
		...
		if (existeFactura(codigoFactura, &factura)) {
			...
			/* El producto no existe */
			printf("\n\tLa factura con el código %d no existe.\n", codigoFactura);
		}
...
void menuModificarF(){
	...
	do {
		system(CLEAR);
		tituloPrincipal();
		printf("\n\t\t\t==> MODIFICAR DATOS DE LA FACTURA POR CÓDIGO <==\n");
		...
		if (existeFactura(codigoFactura, &factura)) {
			...
			printf("\n\tEstá seguro que desea modificar los datos de la factura? [S/N]: ");
			leecad(respuesta, MAX);
...
void reporteFactura(){
	...
	do {
		...
		printf("\n\t\t==> BUSCAR REPORTE DE FACTURA POR CÓDIGO <==\n");
...
void menuInsertarU()
{
	...
	do {
		...
		if (!existeUsuario(codigoUsuario, &usuario)) {
			...
			printf("\tNo puede ingresar dos usuarios distintos con el mismo código.\n");
		}
...
char insertarFactura(Factura factura)
{
	...
	/* Abre el archivo para agregar datos al final */
	archivo = fopen("facturas.dat", "ab");	/* Añade datos al final. Si el archivo no existe, es creado */
